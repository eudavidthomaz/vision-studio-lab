/**
 * MÓDULO DE DETECÇÃO E INTERPRETAÇÃO SEMÂNTICA DE TIPOS DE CONTEÚDO
 * 
 * Este módulo substitui a lógica antiga de detectContentTypes.ts
 * e implementa interpretação semântica inteligente.
 */
...
export function interpretSemanticType(input: string): string {
  ...
  const categoryKeywords: Record<string, { keywords: RegExp[]; defaultType: string }> = {
    biblico: {
      keywords: [/b[ií]blia/i, /vers[ií]culo/i, /deus/i, /jesus/i, /evangelho/i, /ora[cç][aã]o/i, /f[eé]/i, /esp[ií]rito/i],
...
// Função para verificar se é um tipo válido
